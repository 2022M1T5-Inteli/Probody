...
func _on_Area2D_input_event(_viewport, event, _shape_idx):
	...
	if currentHitNumber < maximumHitNumber and event is InputEventMouseButton and event.is_pressed():
		...
		Global.organsUnderAttack[organ]["deathCounter"] += 1.3
		Global.organsUnderAttack[organ]["deathBar"] += ((10.0)/(Global.organsUnderAttack[organ]["deathCounterLimit"] + 1)) * 13
		queue_free() # Deleta a substância que foi clicada
		Global.change_score(1) # Aumenta um ponto
		
		# Ao ser tocado por algum corpo, verifica se o corpo que tocou nele se chama "Celinha"
		# e caso seja, some com o corpo, e adiciona um ponto ao jogador
