...
func _process(_delta):
	...
	if AgeTimer >= 1000:
		...
		if inMain and organsUnderAttack.empty() and currentAge != "idoso":
			...
			get_tree().change_scene("res://LevelUp.tscn")
	...
	if currentAge == "jovem":
		if organsUnderAttack.empty() and not changingAge:
			...
			organsUnderAttack[pick_weighted_random()] = {"carcinogens": 0, 
				"points": 0, "inScene": false, "maxCarcinogens": 10, "timer": Timer.new(), 
				"spawned": 0, "leftover": 0, "cancerCellIsGrowing": false, "deathCounter": 9, "deathCounterLimit": 9, "deathBar": 0}
	...
	if currentAge == "meia-idade":
		if organsUnderAttack.empty():
			...
			for _i in range(2):
				...
				organsUnderAttack[pick_weighted_random()] = {"carcinogens": 0, 
				"points": 0, "inScene": false, "maxCarcinogens": 10, "timer": Timer.new(), 
				"spawned": 0, "leftover": 0, "cancerCellIsGrowing": false, "deathCounter": 19, "deathCounterLimit": 19, "deathBar": 0}
	if currentAge == "idoso":
		ageTimerRunning = false
