...
var organ
var pointSoundFile = load("res://assets/audios/newPoint.wav")
var organscore
...
func _physics_process(delta):
	...
	if collisionInfo and collisionInfo.collider.name == organ: # Se houver colisão
...
func _on_Area2D_input_event(_viewport, event, _shape_idx):
	...
	if currentHitNumber < maximumHitNumber and event is InputEventMouseButton and event.is_pressed():
		Global.play_point_sound() # Toca som de ponto
